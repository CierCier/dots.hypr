...
> The Bootstrap Script Replaces /etc/pacman.conf with a custom one, that uses the Chaotic-AUR repo.
    > The Chaotic-AUR repo is a community maintained repo that provides prebuilt packages for the Arch User Repository.
    > The Chaotic-AUR repo is not officially supported by Arch Linux, and is not recommended for most use cases.
